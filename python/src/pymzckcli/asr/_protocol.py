...
class Config:
    def __init__(self, api_key: str = "", app_key: str = "", access_key: str = "",
...
class AsrWsClient:
    ...
    async def read_audio_data(self, file_path: str) -> bytes:
        try:
            # 统一转换为 DEFAULT_SAMPLE_RATE/16bit/单声道的 WAV,
            # 避免输入 wav 的采样率与请求声明不一致导致识别失败
            logger.debug("Converting audio to unified WAV format...")
            return CommonUtils.convert_wav_with_path(file_path, DEFAULT_SAMPLE_RATE)
        except Exception as e:
